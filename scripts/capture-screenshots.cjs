...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => /[1-9]/.test(document.querySelector("#text-today-tokens").textContent));
    await page.locator("#hourly-chart-container svg").waitFor();
    assert.equal(await page.locator("#quota-source-value").textContent(), "cache");
    assert.match(await page.locator("#text-weekly-status").textContent(), /Cached/);
    ...
    assert.equal(await page.locator("#request-feedback").getAttribute("data-tone"), "warn", "Demo has no credentials: refresh must not claim live quota success");
    assert.equal(await page.locator("#quota-source-value").textContent(), "cache");
    assert.notEqual(await page.locator("#quota-error-value").textContent(), "—");
    await page.unroute("**/api/quota?force=true");
    const demoQuota = await (await page.request.get("http://127.0.0.1:10201/api/quota")).json();
    await page.route("**/api/quota?force=true", (route) => route.fulfill({ json: { ...demoQuota, source: "wham", updatedAt: Date.now(), errorReason: null } }));
    await page.locator("#btn-refresh").click();
    await page.waitForFunction(() => !document.querySelector("#btn-refresh").disabled);
    assert.equal(await page.locator("#request-feedback").getAttribute("data-tone"), "success", "Only a successful fresh response should report success");
    await page.unroute("**/api/quota?force=true");
    await page.route("**/api/history?*", async (route) => {
